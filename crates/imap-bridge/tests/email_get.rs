...
fn stable(rig: &Rig, list: &Value) -> Value {
    ...
    let names = stable_ids::names(rig, &emails);
    let mut list = Value::Array(emails);
    stable_ids::rename(&mut list, &names);
    list
}

async fn get(rig: &Rig, arguments: &Value) -> Value {
...
async fn a_blob_the_host_does_not_open_is_a_server_failure_for_that_call() {
    ...
    let answer = get(&locked, &json!({ "accountId": ACCOUNT, "ids": ids })).await;
